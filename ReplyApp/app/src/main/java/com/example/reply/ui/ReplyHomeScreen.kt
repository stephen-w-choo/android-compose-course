...
fun ReplyHomeScreen(
    ...
    // using an if statement and a boolean value held in the state object
    // changes in the state object will trigger recomposition
    // essentially a substitute to a navcontroller - with only 2 screens we don't need to maintain
    // the backstack and the switch statements
    // we have to manually add in back button functionality here
    if (navigationType == ReplyNavigationType.PERMANENT_NAVIGATION_DRAWER
//        && replyUiState.isShowingHomepage
//        this additional condition is not required - if there's a permanent navigation drawer
//        then ReplyAppContent will be in its expanded form and will not need to change to ReplyDetailsScreen
    ) {
